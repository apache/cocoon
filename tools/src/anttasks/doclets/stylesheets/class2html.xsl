...
<!-- $Id: class2html.xsl,v 1.1 2004/05/25 12:53:43 gcasper Exp $ -->
...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
  ...
  <xsl:template match="/jd:class">
    ...
    <dl>
      ...
      <dt>
        ...
        <a href="{concat('/', $rooturl, '/', translate(jd:inherit/@qname, '.', '/'), '.html')}">
      ...
      <xsl:if test="jd:implements/*">
        <dt>
          ...
          <xsl:for-each select="jd:implements/jd:interface">
            <a href="{concat('/', $rooturl, '/', translate(@qname, '.', '/'), '.html')}">
              <xsl:value-of select="@name" />
            </a>
  ...
  <xsl:template match="jd:inherit" mode="tree">
    <xsl:for-each select="descendant-or-self::jd:inherit">
      ...
      <a href="{concat('/', $rooturl, '/', translate(@qname, '.', '/'), '.html')}"><xsl:value-of select="@qname" /></a>
      <xsl:text>&#xA;</xsl:text>
    </xsl:for-each>
  ...
  <xsl:template match="jd:tags">
    <dl>
      ...
      <xsl:if test="jd:author">
        ...
        <dd>
          <xsl:for-each select="jd:author/jd:comment">
      ...
      <xsl:if test="jd:cocoon.usage">
        <dt>
          <b>Usage:</b>
        </dt>
        <dd>
          <xsl:for-each select="jd:cocoon.usage/jd:comment">
            <xsl:value-of select="text()" disable-output-escaping="yes" />
            <xsl:if test="position() != last()">
              <xsl:text>, </xsl:text>
            </xsl:if>
      ...
      <tr bgcolor="#EEEEFF" class="TableSubHeadingColor">
        <td>
          <b>
            ...
            <a href="{concat('/', $rooturl, '/', translate(@qname, '.', '/'), '.html')}"><xsl:value-of select="@name" /></a>
          </b>
        </td>
      </tr>
      <tr bgcolor="white" class="TableRowColor">
      ...
      <tr bgcolor="white" class="TableRowColor">
        <td>
          <code>
            <xsl:for-each select="jd:field">
              <xsl:sort select="@name" data-type="text" />
              <a href="{concat('/', $rooturl, '/', translate(parent::*/@qname, '.', '/'), '.html', '#', @name)}"><xsl:value-of select="@name" /></a>
              <xsl:if test="position() != last()">
                <xsl:text>, </xsl:text>
              </xsl:if>
            </xsl:for-each>
      ...
      <pre>
        ...
        <xsl:if test="jd:throws/jd:exception">
          ...
          <xsl:for-each select="jd:throws/jd:exception">
            <a href="{concat('/', $rooturl, '/', translate(@name, '.', '/'), '.html')}">
          ...
          <td>
            <code>
              <xsl:for-each select="jd:method[not(@signature = $signatures)]">
                <xsl:sort select="@name" data-type="text" />
                <a href="{concat('/', $rooturl, '/', translate(parent::*/@qname, '.', '/'), '.html', '#', @signature)}"><xsl:value-of select="@name" /></a>
                <xsl:if test="position() != last()">
                  <xsl:text>, </xsl:text>
                </xsl:if>
              </xsl:for-each>
  ...
  <xsl:template name="output-type">
    ...
    <xsl:choose>
      <xsl:when test="contains($type/@type, '.')">
        <!-- Class type: -->
        <a href="{concat('/', $rooturl, '/', translate($type/@type, '.', '/'), '.html')}">
          <xsl:call-template name="class-part">
