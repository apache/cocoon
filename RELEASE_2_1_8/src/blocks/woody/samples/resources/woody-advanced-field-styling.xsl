...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="wi:multivaluefield[wi:styling/@list-type='double-listbox']">
    ...
    <span class="woody-doubleList" title="{wi:hint}">
      <table>
        ...
        <tr>
          ...
          <td>
            ...
            <xsl:text>&#160;</xsl:text>
            <input type="button" value="&gt;" onclick="opt{generate-id()}.woody_transferRight()"/>
            <xsl:text>&#160;</xsl:text>
            <br/>
            <xsl:text>&#160;</xsl:text>
            <input type="button" value="&gt;&gt;" onclick="opt{generate-id()}.woody_transferAllRight()"/>
            <xsl:text>&#160;</xsl:text>
            <br/>
            <xsl:text>&#160;</xsl:text>
            <input type="button" value="&lt;" onclick="opt{generate-id()}.woody_transferLeft()"/>
            <xsl:text>&#160;</xsl:text>
            <br/>
            <xsl:text>&#160;</xsl:text>
            <input type="button" value="&lt;&lt;" onclick="opt{generate-id()}.woody_transferAllLeft()"/>
            <xsl:text>&#160;</xsl:text>
            <br/>
      ...
      <script type="text/javascript">
        var opt<xsl:value-of select="generate-id()"/> = woody_createOptionTransfer('<xsl:value-of select="@id"/>', <xsl:value-of select="wi:styling/@submit-on-change = 'true'"/>);
      </script>
    </span>
  </xsl:template>

  <xsl:template match="wi:multivaluefield/wi:styling[@list-type='double-listbox']/@submit-on-change" mode="styling"/>
