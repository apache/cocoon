...
public class LoggerImpl extends AbstractLogEnabled
    ...
    protected int getLevel() {
        if (logger == null) {
            if (LOGGER == null) {
                // Complain
                System.err.println(LoggerImpl.class.getName() +
                                   ": Logger was not initialized, OJB logging disabled! " +
                                   "This logger implementation can be used only " +
                                   "in context of Cocoon, and requires LoggerImpl " +
                                   "component declared in cocoon.xconf.");
